...
public class DataParser {
	...
	public void writeJSONLines(ClusterList<ClusterPoint> l) throws IOException{

		String JSON1 = "{  \"type\": \"Feature\",  \"geometry\": ";
		String JSON2 = ",  \"properties\": {    \"name\": \"";
		String JSON3 = "\"  }}";
		
		BufferedWriter bw = new BufferedWriter(new FileWriter("output/"+l.getName().replaceAll("/", "-")+".json"));
		bw.write(JSON1+l.getGeoJSONLineString()+JSON2+l.getName()+JSON3);
		bw.close();
		System.err.println(JSON1+l.getGeoJSONLineString()+JSON2+l.getName()+JSON3);
		
	}

	public ArrayList<MartaStationWithCrimes> readStations()
			throws NumberFormatException, IOException {

		String token = ",";
		String csv = "data/marta.csv";
		...
		while ((line = br.readLine()) != null) {
			...
			stations.add(new MartaStationWithCrimes(lineArray[0], lineArray[1],
					lineArray[2], Integer.parseInt(lineArray[3]), Double
							...
							.parseDouble(lineArray[5]),
					new ArrayList<CrimeSummary>()));
		}
		br.close();
		return stations;
	}
	public ArrayList<MartaCluster> getStationClusters()
			throws NumberFormatException, IOException {

		String token = ",";
		String csv = "data/marta.csv";
		BufferedReader br = null;
		ArrayList<MartaCluster> stations = new ArrayList<MartaCluster>();
		...
		while ((line = br.readLine()) != null) {
			...
			for (String s : lineArray)
				if (s == null || s.equalsIgnoreCase("NULL"))
					s = "";
			stations.add(new MartaCluster(lineArray[0], lineArray[1],
					lineArray[2], Integer.parseInt(lineArray[3]), Double
							.parseDouble(lineArray[4]), Double
							.parseDouble(lineArray[5]),
					new ArrayList<CrimeCluster>()));
		}
		br.close();
		return stations;
	}
	...
	public ArrayList<CrimeCluster> getCrimeClusters()
			throws NumberFormatException, IOException, ParseException {
		String token = ",";
		String csv = "data/cobra_data.csv";
		BufferedReader br = null;
		ArrayList<CrimeCluster> crimes = new ArrayList<CrimeCluster>();
		...
		while ((line = br.readLine()) != null) {
			...
			if (lineArray.length == 23) {
				...
				CrimeCluster c = new CrimeCluster(offenceId, reportDate,
						occurDate, possDate, beat, address, uCR, dispoCode,
						numVictims, locType, crimeType, nPU, longitude,
						latitude);
				crimes.add(c);
